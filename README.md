...
- **[canary-publish](./skills/git/canary-publish/SKILL.md)**: Publish a giselle-mui-poc canary snapshot to GitHub Packages and bump a consuming app onto it, verifying the app still resolves cleanly. Two phases (publish, bump) so either can run alone once CI auto-publishing is back on.
