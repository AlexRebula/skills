...
| [`/morning-pr-sweep`](./skills/engineering/morning-pr-sweep/SKILL.md)                   | Clear all open PR review debt across your repos and configured orgs in one morning session — discover all open PRs, triage all threads before touching any code, batch fixes into one commit per PR, post SHA confirmations, and report which PRs are merge-ready. |
