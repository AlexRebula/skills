...
- **[canary-publish](./skills/git/canary-publish/SKILL.md)**: Publish a Changesets snapshot/canary release from a source package repo and bump a configured consumer app onto it, verifying it still resolves cleanly. Repo paths, package name, and verify commands come from a bootstrapped config, not hardcoded. Two phases (publish, bump) so either can run alone once CI auto-publishing is back on.
- **[check-pr-link](./skills/git/check-pr-link/SKILL.md)**: Read-only verification that a PR is actually linked to the issue it implements and that the issue's acceptance-criteria checkboxes reflect what the PR's own checklist confirms. Never edits anything.
...
- **[dependabot-sweep](./skills/git/dependabot-sweep/SKILL.md)**: Classify every open Dependabot PR by real risk (dev-only vs. shipped dependency × bump size, not semver alone), batch the low-risk ones into one combined PR tested with a single quality-gate run, and leave high-risk ones open with a one-time explanatory comment.
- **[link-pr-to-issue](./skills/git/link-pr-to-issue/SKILL.md)**: Link a PR to the issue it implements — a native `Closes #N` keyword when same-repo, a comment on the issue's own repo when cross-repo (the only mechanism that can't leak a private repo's name into a public PR). Resolves the issue from the branch's ticket number.
- **[manual-vercel-deploy](./skills/git/manual-vercel-deploy/SKILL.md)**: Deploy an app straight to Vercel via the Vercel CLI, bypassing GitHub Actions and Vercel's own git-integration build entirely. Use whenever GitHub Actions is unavailable (an Actions spending limit being reached, most commonly) and a real deploy still needs to go out.
...
- **[reap-ticket-branches](./skills/git/reap-ticket-branches/SKILL.md)**: Find local git branches and worktrees left behind by a ticket closed without a merged PR of its own, and report what is safe to remove. Read-only: prints findings and the exact git commands to run, classified against GitHub when a remote is available, never deletes anything itself.
...
- **[sync-down](./skills/git/sync-down/SKILL.md)**: Pull incoming changes from your configured source repo into your working repo, gated by your own quality-gate command. Never commits.
...
- **[sync-up](./skills/git/sync-up/SKILL.md)**: Promote a chosen file list from your working repo into your target repo via a reviewed PR, gated by your own quality-gate command, a built-in privacy scan, and an optional banned-content check. Never merges.
