...
- [ ] Run `/ingest` on a source containing PII and verify Step 2.5 fires and replacements are applied before any file is written
- [ ] Run `/ingest` on a source with a private individual's name and verify slug uses role-based label, not the name
- [ ] Run `/write-a-skill` and verify new checklist item appears in the review step
