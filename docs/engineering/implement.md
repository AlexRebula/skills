...
| Already built, and you want it checked | [review-pr](../git/review-pr.md) directly |
...
If the tickets came from [to-tickets](https://aihero.dev/skills-to-tickets), the tracker they live on was configured by `setup-engineering-skills`. `review-pr` reads the same configuration to find the originating spec at close-out.
...
5. Run [review-pr](../git/review-pr.md) in branch mode, then commit to the current branch.
...
- **Four pre-flight checks**, run before any code gets written: a ready-state check (stop if the ticket's still `to-grill` or `needs-info`), a blocker check (verify every `## Blocked by` reference is actually closed), an existing-work check (look for a branch or open PR already covering this ticket before starting fresh), and an existing-equivalent check (search the actual codebase for a component that might already satisfy the ticket under a different name, rather than trusting the ticket's own "this is unbuilt"/"distinct from X" claims at face value). The first three absorb what [start-issue](https://aihero.dev/skills-start-issue) used to do on this fork. `start-issue` is now deprecated: its only real value beyond what these checks and the fetch-in-full step already cover was a triage-routing decision, and the user who'd have used it for that was routing through `/implement` directly anyway.

## Common questions

**It finished, but my ticket is still open and the acceptance criteria are still unchecked.**

Correct, and expected. `implement` has no completion step. It ends at the commit and never touches the work item, confirmed on GitHub Issues and on the local markdown tracker, so it is not a tracker integration problem. It also does not act on the findings `review-pr` produced, and does not tick the `- [ ]` boxes on the originating issue. Close the ticket and reconcile the criteria yourself. This bites hardest on a dependency chain, because `to-tickets` defines the frontier as tickets whose blockers are all closed. If nothing gets closed, nothing ever becomes visibly unblocked.

**Can I point it at all my tickets at once, or run several in parallel?**

Not in parallel — [subagent](https://www.aihero.dev/ai-coding-dictionary/subagent) fan-out is requested repeatedly and does not exist. Running several `/implement` sessions side by side in one checkout is worse than unsupported: one field report describes a `git commit --amend` in one session landing on another session's commit, a stash vanishing from `refs/stash`, and commits landing on the wrong branch, all in a single afternoon across three issues. The sessions share one working directory, one index, and one HEAD. Git worktrees are the community workaround, and note that `refs/stash` is shared across worktrees too, so worktrees alone do not fix the stash case.

Sequentially, though, [implement-tickets](https://aihero.dev/skills-implement-tickets) now does exactly this: given a parent issue with GitHub-native sub-issues, it loops `/implement` across the children one at a time in dependency order, stacking branches for in-batch dependencies instead of waiting for merges, until the whole batch is built.
...
**`review-pr` says it cannot see my changes.**

`review-pr --branch` reviews `git diff <default-branch>...<branch>`, which excludes staged and working-tree changes. `implement` runs it before committing, so unless an interim commit already exists there is nothing in that diff to review. This was a reported limitation of `code-review`, the skill `review-pr` replaced, and the same shape carries over here. Commit first, then review against the point you branched from.

Separately, some people deliberately do not want the review inside the run at all, because an agent reviewing the code it just wrote is biased toward its own solution. Running [review-pr](../git/review-pr.md) in a fresh session is a legitimate alternative, and is the same reason that skill runs its two axes in separate sub-agents.
