...
Skills live in bucket folders under `skills/`: `engineering/`, `framework/`, `git/`, `org/`, and `productivity/` are **promoted** (shipped); `misc/`, `personal/`, `in-progress/`, and `deprecated/` are **not**. A plugin must expose only the promoted set, which spans five of those bucket folders.
...
Upstream `mattpocock-skills` was later accepted into Claude Code's official marketplace (`claude-plugins-official`), which every Claude Code install has by default. That listing is upstream's own, keyed to `mattpocock/skills`'s git URL; it does not extend to this fork, and `alexrebula-skills` has no equivalent official listing.

For this repo, the `marketplace add` → `install` path above (via `.claude-plugin/marketplace.json`) is still the documented route, not a fallback: `/plugin marketplace add AlexRebula/skills` then `/plugin install alexrebula-skills@AlexRebula`. The install wording lives in [.agents/install-block.md](../install-block.md).
