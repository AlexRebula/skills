...
- **[migrate-giselle-subcomponent](./migrate-giselle-subcomponent/SKILL.md)**: Migrate an existing, already-implemented flat sub-component into its own Scenario A subfolder per `cleanup-workflow.md` — a mechanical move of working code, not a scaffold-from-scratch rebuild.
- **[port-giselle-component](./port-giselle-component/SKILL.md)**: Bring an already-built, already-tested component in from an external repo without a from-scratch TDD rebuild. Guarded: only applies when the source's existing tests pass in their new home with import-path edits alone; any behavioral change required hands off to `create-giselle-component` instead.
