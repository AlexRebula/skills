...
- **[pr-merged](./pr-merged/SKILL.md)**: Post-merge cleanup for one PR already merged on GitHub with its remote branch already deleted: verify it shows `MERGED`, close the linked issue with a summary comment, delete the local branch/worktree, prune the remote-tracking ref, fast-forward the base branch.
