...
Three points in this workflow are opinionated. They are called out inline with **⚙️ Configurable** labels.

| # | Default behaviour | What to override |
|---|---|---|
| 1 | Quality gate command: `npm run check:verify` | Replace with any command that exits 0 on pass |
| 2 | PR description uses `.github/pull_request_template.md` if present; falls back to a conventional set of sections | Provide your own template or skip the fallback |
| 3 | Review trigger: **off** by default | Pass `request-review` argument to trigger GitHub Copilot reviewer |
