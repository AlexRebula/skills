...
description: >
  ...
  a new categorised branch (feature/, fix/, docs/, chore/, data/, wip/) when no remote
...
2. **Today's WIP branch** — a `wip/YYYYMMDD*` branch where the date is today's date
...
| `wip` | `wip/YYYYMMDD-<dominant-topic>` | `wip/20260523-mixed-config` |
...
# Commit with a meaningful WIP message
git -C <repo-path> commit -m "WIP: <group label> — $(date +%Y-%m-%d)"
...
- WIP commit messages (`"WIP: ..."`) are easy to grep. Squash or amend before opening a PR.
