...
**Visual-comparison acceptance criteria must name the human, never leave it ambiguous.** If a criterion asks for anything visual — "renders/behaves identically to the reference," a screenshot check, "looks right" — write it as `(verified by the repo owner — never the implementing agent)`, not bare prose like "verified via screenshot comparison." Bare phrasing like that has already been read as agent-performed at least once, which is unreliable and slower than the human just doing it (see `/implement`'s own "Visual verification is never agent-performed" rule, which this wording feeds). This isn't optional polish — an ambiguous criterion here is exactly what causes it to be skipped there.
