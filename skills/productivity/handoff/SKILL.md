...
times before the handoff was called. Work done in earlier parts of the session may have been
...
2. Extract every `<conversation-summary>` compaction block. Each block is a faithful summary
   of what happened before that compaction point.
...
If no transcript file is available, proceed from context alone and add this note to the
handoff document:

> ⚠️ Session transcript unavailable — this handoff may be incomplete if the conversation
> was compacted. Verify against screenshots or a separate session log if needed.

---

## Step 2 — Write the handoff document

Write a handoff document summarising the full session history (from Step 1) so a fresh agent
