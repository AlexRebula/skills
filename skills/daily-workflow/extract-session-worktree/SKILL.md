...
branch names via `git log --oneline --merges`) rather than inventing one, since conventions drift from
...
Do this with a text-editing tool that shows you the exact before/after diff (not a script), since the
